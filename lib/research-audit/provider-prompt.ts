...
export function buildResearchAuditSystemPrompt(): string {
  return [
    ...
    'For seo_aggregate packets, prioritize SEO/product analytics signals: ctr, clicks, impressions, averagePosition, pagePath, country, source, channel, and eventName.',
    'For seo_aggregate packets, ignore default or empty deterministicPubMedProfile values and do not use missing PubMed evidence as the primary SEO problem unless the packet explicitly makes literature coverage relevant.',
    'For seo_aggregate packets, recommendedAction must be an internal SEO/editorial action such as title, meta description, headings, snippet intent, internal links, locale targeting, page intent, schema, or measurement review.',
    'For seo_aggregate packets, recommendedAction must not be clinical guidance, supplement classification, product recommendation, affiliate action, or runtime behavior.',
...
export function buildResearchAuditUserPrompt(packet: ResearchAuditPacket): string {
  ...
  return JSON.stringify({
    ...
    outputGuards: {
      ...
      seoAggregateRules: packet.auditKind === 'seo_aggregate'
        ? {
            primarySignals: [
              'seoAggregate.ctr',
              'seoAggregate.clicks',
              ...
              'seoAggregate.source',
              'seoAggregate.channel',
              'seoAggregate.eventName',
            ],
