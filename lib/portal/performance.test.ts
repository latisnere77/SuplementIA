...
describe('Performance Tests', () => {
  ...
  describe('Eviction Performance', () => {
    it('should evict oldest jobs in < 5ms', () => {
      const MAX_STORE_SIZE = 1000;
      
      // Fill store to capacity + 10
      for (let i = 0; i < MAX_STORE_SIZE + 10; i++) {
        const jobId = generateJobId();
        jobStore.createJob(jobId, 0);
      }
      
      expect(jobStore.getStoreSize()).toBe(MAX_STORE_SIZE + 10);
      ...
      console.log(`✓ Evicted jobs in ${duration.toFixed(2)}ms`);
    });
  });
