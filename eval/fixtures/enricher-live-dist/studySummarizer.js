...
exports.summarizeStudies = summarizeStudies;
exports.formatSummariesForPrompt = formatSummariesForPrompt;
 ...
 */
async function summarizeSingleStudy(study) {
...
Summary:`;
    try {
        const command = new client_bedrock_runtime_1.InvokeModelCommand({
            modelId: config_1.config.modelId, // Use Haiku for speed
...
async function summarizeStudies(studies) {
    ...
    try {
        ...
        const summaries = await Promise.all(studies.map(study => summarizeSingleStudy(study)));
        const duration = Date.now() - startTime;
        // Calculate token reduction
        const originalTokens = studies.reduce((sum, s) => sum + (s.abstract?.length || 0) + (s.findings?.length || 0), 0) / 4; // Rough estimate: 1 token ≈ 4 chars
        const summaryTokens = summaries.reduce((sum, s) => sum + s.summary.length, 0) / 4;
...
function formatSummariesForPrompt(summaries) {
    return summaries.map((s, i) => `Study ${i + 1} (PMID: ${s.pmid}, ${s.year || 'N/A'}):
${s.summary}`).join('\n\n');
}
//# sourceMappingURL=studySummarizer.js.map