...
exports.generateEnrichedContent = generateEnrichedContent;
const client_bedrock_runtime_1 = require("@aws-sdk/client-bedrock-runtime");
const aws_xray_sdk_core_1 = __importDefault(require("aws-xray-sdk-core"));
const config_1 = require("./config");
const prompts_1 = require("./prompts");
const prompts_examine_style_1 = require("./prompts-examine-style");
...
const client = config_1.config.xrayEnabled
    ? aws_xray_sdk_core_1.default.captureAWSv3Client(baseClient)
    : baseClient;
/**
 * Call Bedrock to generate enriched content
 */
async function generateEnrichedContent(supplementId, category = 'general', studies, contentType = 'standard', benefitQuery) {
    const startTime = Date.now();
    // Build prompt based on content type
    const prompt = contentType === 'examine-style'
        ? (0, prompts_examine_style_1.buildExamineStylePrompt)(supplementId, category, studies)
        : (0, prompts_1.buildEnrichmentPrompt)(supplementId, category, studies, benefitQuery);
    ...
    const response = await (0, retry_1.retryWithBackoff)(async () => {
        return await client.send(new client_bedrock_runtime_1.InvokeModelCommand({
            modelId: config_1.config.modelId,
    ...
    const percentageUsed = (responseBody.usage.output_tokens / config_1.config.maxTokens * 100);
    console.log(JSON.stringify({
    ...
    const parseJSONWithFallback = (text) => {
        ...
        if (firstBrace !== -1 && lastBrace !== -1 && lastBrace > firstBrace) {
            ...
            try {
                ...
                const snippet = sanitizedExtracted.substring(Math.max(0, errorPos - 50), Math.min(sanitizedExtracted.length, errorPos + 50));
                // Log character codes around error position for debugging Unicode issues
                const charCodes = sanitizedExtracted.substring(Math.max(0, errorPos - 5), Math.min(sanitizedExtracted.length, errorPos + 5)).split('').map(c => c.charCodeAt(0));
        ...
        throw new Error('Failed to parse JSON from Bedrock response after all repair strategies. ' +
    ...
    try {
        ...
        throw new Error(`Failed to parse enriched content JSON: ${errorMessage}. ` +
    ...
    const validation = contentType === 'examine-style'
        ? (0, prompts_examine_style_1.validateExamineStyleContent)(enrichedData)
        : (0, prompts_1.validateEnrichedContent)(enrichedData);
    if (!validation.valid) {
