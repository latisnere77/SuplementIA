        ...
        return NextResponse.json(
          {
            ...
            suggestion: errorData.suggestion || 'Intenta con una forma o extracto específico, verifica la ortografía, o explora un tema clínico o componente específico.',
            requestId,
            category: sanitizedCategory,
        ...
        return NextResponse.json(
          {
            ...
            suggestion: enrichData.suggestion || 'Intenta con una forma o extracto específico, verifica la ortografía, o explora un tema clínico o componente específico.',
            requestId,
            category: sanitizedCategory,
...
function isCentellaContent(content: any, category: string): boolean {
...
function sanitizeCentellaClaimText(value: unknown): unknown {
  ...
  return value
    ...
    .replace(/\b(?:60-70|60-80|40-50|25-35|30-40|20-30|5-15|10-20)%\b/gi, 'mejoras reportadas');
}

function sanitizeCentellaItem<T>(item: T): T {
...
function isCentellaPreliminaryWorksFor(item: any): boolean {
...
function isCentellaVenousOrWoundItem(item: any): boolean {
  const text = normalizeClinicalText([
    item?.condition,
    item?.notes,
  ].filter(Boolean).join(' '));
...
function calibrateCentellaWorksFor(content: any) {
  const worksFor = Array.isArray(content.worksFor) ? content.worksFor : [];
  const limitedEvidence = Array.isArray(content.limitedEvidence) ? [...content.limitedEvidence] : [];
  const calibratedWorksFor: any[] = [];

  for (const rawItem of worksFor) {
    const item = sanitizeCentellaItem({ ...rawItem });

    if (isCentellaPreliminaryWorksFor(item)) {
      limitedEvidence.push({
        ...item,
        evidenceGrade: 'C',
        grade: 'C',
        condition: sanitizeCentellaClaimText(item.condition) || 'Evidencia preliminar en Centella asiatica',
        notes: `${sanitizeCentellaClaimText(item.notes) || ''} Evidencia preliminar: no debe presentarse como beneficio clínico confirmado.`.trim(),
        magnitude: 'Evidencia preliminar; magnitud clínica no establecida.',
      });
      continue;
    }
  ...
  return {
    ...
    limitedEvidence: sanitizeCentellaItem(limitedEvidence),
  };
}
...
function calibrateCentellaEnrichedContent(content: any, category: string): any {
  if (!content || !isCentellaContent(content, category)) return content;

  const calibrated = sanitizeCentellaItem({ ...content });
  const { worksFor, limitedEvidence } = calibrateCentellaWorksFor(calibrated);
  ...
  if (calibrated.dosage) {
    calibrated.dosage = sanitizeCentellaItem(calibrated.dosage);
  }
