...
// weaviate-authorizer.ts
var weaviate_authorizer_exports = {};
__export(weaviate_authorizer_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(weaviate_authorizer_exports);
...
// weaviate-authorizer.ts
var COGNITO_REGION = process.env.COGNITO_REGION || "us-east-1";
var COGNITO_USER_POOL_ID = process.env.COGNITO_USER_POOL_ID || "us-east-1_u4IwDoEbr";
var COGNITO_CLIENT_ID = process.env.COGNITO_CLIENT_ID || "1l1o4bh4q3v1kkvjmupeek2dl8";
var COGNITO_ISSUER = `https://cognito-idp.${COGNITO_REGION}.amazonaws.com/${COGNITO_USER_POOL_ID}`;
var JWKS_URI = `${COGNITO_ISSUER}/.well-known/jwks.json`;
var JWKS = createRemoteJWKSet(new URL(JWKS_URI));
var handler = async (event) => {
  const headers = event.headers;
  const authHeader = headers?.authorization || headers?.Authorization;
  const auditLog = (authorized, reason, email) => {
